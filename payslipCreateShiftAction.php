...
$shift_type = intval($_POST['shift_type'] ?? 0);
  ...
  $sql = "INSERT INTO shift_records (user_code, shift_type, date, s_time, e_time, break, rate, l_allowance, u_allowance, shift_allow, is_holi) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  $stmt = mysqli_prepare($conn, $sql);

  if($stmt){
    // types: user_code (s), shift_type (i), date (s), start_time (s), end_time (s), break (d), rate (d), laundry (d), uniform (d), shift_allow (d), is_holi (i)
    mysqli_stmt_bind_param($stmt, "sisssdddddi", $user_code, $shift_type, $date, $start_time, $end_time, $breaks, $rate, $laundry, $uniform, $shift_allow, $is_holi);
